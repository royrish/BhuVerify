...
async def list_documents(limit: int = Query(default=100, ge=1, le=500), offset: int = Query(default=0, ge=0)):
    ...
    return [
        {**document, **records_by_document.get(document["id"], {})}
...
async def run_document_ocr(document_id: str):
    ...
    try:
        supabase.table("documents").update({"language": result.get("language_code", "unknown")}).eq("id", document_id).execute()
    except Exception as error:
...
async def verify_document(document_id: str, request: VerifyRequest):
    ...
    try:
        ...
        core_complete = all(updated_record.get(field) is not None for field in (
            "owner_name", "survey_number", "khata_number", "area", "village", "tehsil", "district"
        ))
