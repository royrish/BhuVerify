...
export default function SettingsPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (loginRole === "citizen") {
      ...
      setMessage("Logged in successfully as Government Officer.");
    }
    setNameInput("");
    setEmailInput("");
    setIdInput("");
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
            ...
            {currentUser ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontWeight: 700, color: currentUser.role === "official" ? "#0369a1" : "#065f46" }}>
                      {currentUser.role === "official" ? "Government Official" : "Citizen / Landholder"}
                    </div>
                  </div>
                  <div>
                ...
                <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    ...
                    onClick={() => {
                      if (currentUser.role === "official") {
                        ...
                        const updated: AuthUser = {
                          ...currentUser,
                          role: "official",
                          identifier: "REV-OFFICER-551",
                          department: "Revenue Administration Department",
                        };
                        localStorage.setItem("bhuverify_user", JSON.stringify(updated));
                        setCurrentUser(updated);
                        setMessage("Switched active profile to Official.");
                      }
                    }}
                    style={{
                      flex: 1,
                  ...
                  >
                    {currentUser.role === "official" ? "Switch to Citizen Page" : "Switch to Official Profile"}
                  </button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder={loginRole === "official" ? "e.g. Officer Ramachandran" : "e.g. Ramesh Patel"}
                    value={nameInput}
                ...
                <div>
                  <label style={{ fontSize: "11px", fontWeight: 700, color: "#475569" }}>Reference ID</label>
                  <input
                    type="text"
                    required
                ...
                >
                  {loginRole === "citizen" ? "Login & Go to Citizen Portal" : "Authenticate as Official"}
                </button>
              </form>
            )}
          </div>
          ...
          <div className={styles.panel} style={{ padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              <label style={{ fontSize: "12px", fontWeight: 700, color: "#334155" }}>Backend API Gateway</label>
              <input
                type="text"
                readOnly
                value="http://127.0.0.1:8001/api"
                style={{ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", padding: "8px 10px", borderRadius: "6px", fontFamily: "monospace", fontSize: "12px", color: "#475569" }}
              />
            </div>
          </div>
